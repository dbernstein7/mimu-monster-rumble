...
import { dedupeBestScorePerUser } from './dedupe.js';
...
export async function handleGet(query) {
  ...
  try {
    const raw = await zrevrangeWithScores(SCORES_KEY, MAX_ENTRIES);
    const entries = [];
    ...
    const deduped = dedupeBestScorePerUser(entries).slice(0, limit);

    return {
      status: 200,
      json: { ok: true, configured: true, entries: deduped, updatedAt: Date.now() },
    };
  } catch (e) {
    return {
      status: 200,
  ...
  try {
    ...
    try {
      ...
      if (metaRaw) {
        ...
        existingScore = parseScore(existing?.score);
      }
    } catch {
      existingScore = null;
    }
    ...
    if (existingScore !== null && score < existingScore) {
      return {
        status: 200,
        json: { ok: true, configured: true, saved: false, skipped: 'not_personal_best' },
      };
    }
    ...
    if (existingScore !== null && score === existingScore) {
      let zsetScore = null;
      try {
        zsetScore = parseScore(await zscore(SCORES_KEY, member));
      } catch {
        zsetScore = null;
      }
      if (zsetScore !== null && zsetScore >= score) {
        return {
          status: 200,
          json: { ok: true, configured: true, saved: false, skipped: 'not_personal_best' },
        };
